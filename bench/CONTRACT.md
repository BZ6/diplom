...
```json
{
  ...
  "hops":           ["!000003e9", "!000003eb", "!000003ea"],
...
| `hops` | array `!hex[]` | да | Полный вычисленный путь от отправителя до `for_destination` (включительно). `hops[0]` — отправитель, `hops[-1]` — пункт назначения. Узел следует этому маршруту по порядку. |
| `use_next_hop` | string `!hex` | да | Следующий прыжок = `hops[1]` (удобный доступ без разбора массива) |
